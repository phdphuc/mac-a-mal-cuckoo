...
class Macalyzer(object):
    ...
    def _analysis(self, package):
        ...
        import shlex
        command_line = "codesign -dvvvv \"%s\"" % self.target
        args = shlex.split(command_line)
        p1 = Popen(args, stderr=PIPE)
        self.log.info("%s", p1.communicate()[1])
        package.start()
    ...
    def _upload_file(self, filepath):
        ...
        if (filepath.startswith(analyzerpath) or filepath.startswith(resultpath)):
